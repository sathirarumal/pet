(function () {
    'use strict';
    angular.module('app').controller('ReportingHierarchyController', ['$scope', 'dataService', '$rootScope', function ($scope, dataService, $rootScope) {
        var vm = this;
        ...
        function onInit() {
            dataService.browseReportingHierarchy().then(function (resp) {
                vm.treeData = resp;
                vm.treeIsReady = true;
            }).catch(function (err) {
                console.error(err);
            });
            dataService.browseReportingHierarchyList().then(function (resp) {
        ...
        function addLayer(parent_id, status, depth) {
            var $modal = $('#myModal');
            $modal.load(http_path + '/hierarchy/AddLayerBox', {
                    parent_id: parent_id,
                    status: status,
                    depth: depth
                },
        ...
        function hierarchyDetails(parent_id) {
            var $modal = $('#myModal');
            $modal.load(http_path + '/hierarchy/hierarchyDetails', {parent_id: parent_id},
                function () {
                    $modal.modal('show');
                    form.reset();
                });
        ...
        function deleteLayer(id) {
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        $.ajax({
                            url: http_path + 'index.php/Hierarchy/deleteLayer',
                            data: "id=" + id,
                            type: 'POST',
